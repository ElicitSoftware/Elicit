...
Site 2's host ports are site 1's minus 50. Neither site publishes SMTP or the Jaeger collector
ports: the apps reach those services by compose service name. Neither site runs FHHS: it serves
only the Family History Survey and this journey runs the Household Survey, so it would never
become ready (FHHS UC-005). The SFTP server went with it, since nothing else used it. Site 2 shares site 1's
...
0. Each site starts with no department (Admin UC-028). The first admin visit to each site
   follows the blocking dialog's remedy and creates the site's department, which Admin assigns
   to its creator.
...
2. Site 1 creates department "Site 1 Clinic" (code `SITE1`) and applies v1 ("New Survey Installed").
3. Site 2 creates department "Site 2 Clinic" (code `SITE2`), which is assigned to `admin` on creation, and applies v1.
...
12. Site 1 creates a local department with site 2's code and imports the four files.
...
- Docker Desktop; the module images built from the umbrella root (`./buildDockerImages.sh`).
  FHHS and Pedigree are not needed by this journey (FHHS is not in these stacks at all).
...
- The reporting star (`surveyreport`) is only populated for survey id 1; the imported
  respondents are complete in `survey.*` but do not appear in reporting.
